...
const sensitiveKey =
	/(?:secret|password|token|authorization|auth|cookie|credential|session|database(?:url)?|jid|qr|message|text|ciphertext|key|body|prompt|response)/i;
const sensitiveValue =
	/(?:postgres(?:ql)?:\/\/|@s\.whatsapp\.net|^data:image\/|^\+?[1-9]\d{7,14}$)/i;
type RedactedValue =
	| string
	| number
	| boolean
...
export function redactOperationsValue(value: unknown, key = ""): RedactedValue {
	...
	if (Array.isArray(value))
		return value.map((item) => redactOperationsValue(item));
	if (value && typeof value === "object")
		return Object.fromEntries(
			Object.entries(value).map(([name, item]) => [
				name,
				redactOperationsValue(item, name),
			]),
		);
	return value === null ||
		typeof value === "number" ||
...
export function serializeOperationalLog(input: {
	...
	return JSON.stringify({
		...
		...(input.details === undefined
			? {}
			: { details: redactOperationsValue(input.details) }),
	});
}
...
export function createHeartbeat(options: {
	...
	return {
		...
		isFresh: (at = now()) =>
			lastSeen > 0 && at - lastSeen <= HEARTBEAT_STALE_MS,
	};
}

export function createDurableReadiness(options: {
	persistDraining: () => Promise<void>;
}) {
	let draining = false;
	return {
		ready: () =>
			draining
				? { ready: false as const, code: "draining" }
				: { ready: true as const, code: "ready" },
		markUnready: () => {
			draining = true;
...
export function createLoopbackProbe(options: {
	ready: () => { ready: boolean; code: string };
	port?: number;
}) {
		...
		start: () =>
			new Promise<void>((resolve) => {
				server = createServer((request, response) => {
					const state =
						request.url === "/live"
							? { ready: true, code: "live" }
							: request.url === "/ready"
								? options.ready()
								: undefined;
					response.writeHead(state ? (state.ready ? 200 : 503) : 404, {
						"content-type": "application/json",
					});
					response.end(JSON.stringify(state ?? { code: "not_found" }));
				}).listen({ host: "127.0.0.1", port: options.port ?? 9090 }, resolve);
			}),
		stop: () =>
			new Promise<void>((resolve) =>
...
export function createDrain(options: DrainOptions) {
	...
	const mandatory = async (work: () => Promise<void>) => {
		try {
			await work();
		} catch {
			return;
