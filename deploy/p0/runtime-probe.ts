...
export async function runRuntimeProbe(
  ...
  if (mode === "native") {
    const encoded = await hash("p0-password", {
  ...
  if (mode === "baileys") {
    let options: Record<string, unknown> = {},
      listener: (value: Record<string, unknown>) => unknown = () => undefined;
    let ready!: () => void;
    const socket: BaileysSocket = {
      ...
      ev: {
        on(name, callback) {
          if (name === "connection.update") listener = callback;
        },
      },
      end() {},
    };
    ...
    const lease = await gateway.connect("p0-connection", (event) =>
      events.push(event),
    );
    lease.close();
    return {
  ...
  if (mode === "queue") {
    ...
    if (!connectionString)
      throw new Error("DATABASE_URL is required for queue probe");
    ...
    const published = await boss.send("p0-release-probe", {
      value: "published-and-consumed",
    });
  ...
  if (!writable)
    throw new Error("P0_WRITABLE_PATH is required for filesystem probe");
  writeFileSync(`${writable}/p0-write`, "ok");
  let denied = false;
  try {
    writeFileSync("/opt/agendia/p0-forbidden", "must fail");
  } catch {
    denied = true;
  }
