...
export async function startMessageWorker(
  ...
  await boss.work<PostgresAiJob>("ai-generate", async ([job]) => {
    ...
    await trackLockCleanup(async () => {
      try {
        ...
        console.info(serializeOperationalLog({ level: "warn", service: "message-worker", environment: env.AGENDIA_ENVIRONMENT ?? "development", releaseDigest: env.AGENDIA_RELEASE_DIGEST ?? "development", instanceId: env.AGENDIA_INSTANCE_ID ?? "worker", code: "timeout" }));
      }
  ...
  const readiness = createDurableReadiness({
    persistDraining: () => pools.worker.run(undefined, (repo) =>
      repo.upsertServiceHeartbeat({ service: "message-worker", instanceId, releaseDigest: env.AGENDIA_RELEASE_DIGEST ?? "development", state: "draining" }),
    ),
  });
  const heartbeat = createHeartbeat({
    write: () => pools.worker.run(undefined, (repo) => repo.upsertServiceHeartbeat({ service: "message-worker", instanceId, releaseDigest: env.AGENDIA_RELEASE_DIGEST ?? "development", state: "ready" })),
  });
  ...
  await probe.start();
  const stop = createDrain({ timeoutMs: 90_000, markUnready: () => readiness.markUnready(), stopIntake: async () => {}, stopTimers: async () => heartbeat.stop(), stopProbe: () => probe.stop(), stopRuntime: () => boss.stop(), releaseLocks: async () => {}, awaitInFlightLockCleanup: () => inFlightLockCleanup, closePools: () => pools.end() });
  return { boss, pools, worker, summaryWorker, stop, readiness: () => heartbeat.isFresh() };
}
if (process.env.AGENDIA_RUN_MESSAGE_WORKER === "1") {
  ...
  void runPreflightBeforeActivity(
    ...
    .then((runtime) => {
      ...
      const log = (level: "info" | "warn" | "error", code: "startup" | "draining" | "stopped" | "timeout") => console.info(serializeOperationalLog({ level, service: "message-worker", environment: config.environment, releaseDigest: config.releaseDigest, instanceId, code }));
      log("info", "startup");
      const stop = () => { log("info", "draining"); return void runtime.stop().then(() => log("info", "stopped")).finally(() => process.exit()); };
      process.once("SIGINT", stop);
      process.once("SIGTERM", stop);
    })
    .catch(() => {
      console.info(serializeOperationalLog({ level: "error", service: "message-worker", environment: config.environment, releaseDigest: config.releaseDigest, instanceId: "bootstrap", code: "stopped" }));
      process.exitCode = 1;
    });
}
