  ...
  return {
    ...
    environments: {
      staging: {
        ...
        database: { host: "postgres", name: "agendia_stg", loginPrefix: "agendia_stg_" },
      ...
      production: {
        ...
        database: { host: "postgres", name: "agendia_prod", loginPrefix: "agendia_prod_" },
        criticalSecretHashes: {
...
async function expectCode(action: () => Promise<unknown>, code: string) {
  ...
  await expect(action()).rejects.not.toThrow(/postgres|agendia_|\+1555|[a-f0-9]{64}/i);
}
...
describe("cross-environment release preflight", () => {
  ...
  test("RED: rejects unknown secret-bearing manifest keys without disclosing their values", async () => {
    const unknownTopLevelSecret = "postgresql://agendia_prod_api:top-secret@prod-db/agendia_prod";
  ...
  test("TRIANGULATE: starts API, manager, and worker activity only after preflight", async () => {
    ...
    for (const process of ["api", "whatsapp-manager", "message-worker"] as const) {
