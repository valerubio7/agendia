...
const context = (businessId: string) =>
  tenantContext(testTenantContext(businessId));
...
describe("production PostgreSQL repositories", () => {
  test("persists auth, administration and each tenant runtime aggregate", async () => {
    await pools.admin.run(undefined, async (repo) => {
      ...
      expect((await repo.findIdentity("owner@example.test"))?.business_id).toBe(
        A,
      );
    });
    ...
    await pools.manager.run(context(A), async (repo) =>
      repo.saveConnection(CONNECTION, A),
    );
    const snapshot = await pools.api.run(context(A), (repo) =>
      repo.tenantSnapshot(),
    );
    ...
    expect(
      (await pools.manager.run(context(A), (repo) => repo.connection()))?.id,
    ).toBe(CONNECTION);
  });

  test("fails closed for absent/falsified context and keeps two tenants separate", async () => {
    await pools.api.run(context(B), (repo) =>
      repo.saveProfile(B, "Comercio B"),
    );
    expect(
      await pools.api.run(undefined, (repo) => repo.tenantSnapshot()),
    ).toEqual({ profile: null, assistantActive: null, inbox: 0, outbox: 0 });
    await expect(
      pools.api.run(context(A), (repo) => repo.saveProfile(B, "intrusion")),
    ).rejects.toThrow(/policy|security/i);
    expect(
      (await pools.api.run(context(A), (repo) => repo.tenantSnapshot()))
        .profile,
    ).toBe("Comercio A");
    expect(
      (await pools.api.run(context(B), (repo) => repo.tenantSnapshot()))
        .profile,
    ).toBe("Comercio B");
  });
  ...
  test("rolls mutation, audit and outbox back atomically", async () => {
      ...
      uow.execute(
        context(A),
        {
          ...
          outbox: {
            ...
            stableKey: "profile:rollback",
            payload: { revision: 2 },
          },
        },
        async (repo) => {
          await repo.saveProfile(A, "Must rollback");
    ...
    expect(await pools.api.run(context(A), (repo) => repo.auditCount())).toBe(
      0,
    );
  });
});
