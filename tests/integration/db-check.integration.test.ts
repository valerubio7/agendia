...
import { verifyPostgresMigrations } from "../../packages/test-support/src/index.ts";
...
describe("database migration and drift gate", () => {
  test("migrates two clean PostgreSQL databases and compares their real schemas", async () => {
    const result = await verifyPostgresMigrations({ migrationDirectory: migrations });
  ...
  test("rejects a temporary invalid migration", async () => {
    ...
    await expect(verifyPostgresMigrations({ migrationDirectory: invalid })).rejects.toThrow(/syntax|migration/i);
  }, 120_000);

  test("rejects real schema drift introduced after migration", async () => {
    await expect(verifyPostgresMigrations({
      migrationDirectory: migrations,
      mutateActual: (sql) => sql.unsafe("create table drift_probe (id integer primary key)"),
