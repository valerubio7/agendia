...
test("worker consumes tenant outbox and a pg-boss job", async () => {
  await pools.api.run(context, (repo) =>
    repo.enqueueOutbox(TENANT, "ai.generate", "worker-event", {
      messageId: "m-1",
    }),
  );
  ...
  expect(
    await pools.worker.run(other, (repo) => repo.claimOutbox()),
  ).toBeNull();
  const claimed = await pools.worker.run(context, (repo) => repo.claimOutbox());
  expect(claimed).toMatchObject({
    topic: "ai.generate",
    stable_key: "worker-event",
  });
  expect(
    await pools.worker.run(context, (repo) =>
      repo.markOutboxPublished(claimed!.id),
    ),
  ).toBe(1);

  await boss.createQueue("unit-20");
  const received = new Promise<string>((resolve) =>
    boss.work("unit-20", async ([job]) => {
...
test("worker reads only tenant context while admin and worker cannot read Baileys credentials", async () => {
  await expect(
    pools.admin.run(context, (repo) => repo.conversationTexts()),
  ).rejects.toThrow(/permission denied/i);
  expect(
    await pools.worker.run(context, (repo) => repo.conversationTexts()),
  ).toHaveLength(0);
  for (const pool of [pools.admin, pools.worker])
    await expect(
      pool.run(context, (repo) => repo.authRecordNames()),
    ).rejects.toThrow(/permission denied/i);
});
