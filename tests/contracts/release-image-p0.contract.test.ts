...
describe("P0 release image feasibility contract", () => {
  test("fails closed when the native argon2 asset is deliberately omitted", () => {
    expect(inspectReleaseLayout(completeLayout.filter((path) => !path.includes("argon2"))))
  ...
  test("accepts generated output containing native, dynamic, and static runtime assets", () => {
    expect(nextConfig).toMatchObject({ output: "standalone", outputFileTracingRoot: process.cwd() });
    expect(inspectReleaseLayout(completeLayout)).toEqual({ viable: true, missing: [] });
    expect(Object.values(releaseCommands).every((command) =>
      !/(tsx|next dev|src\/|\.ts\b)/.test(command.join(" ")),
    )).toBe(true);
  });
  ...
  test("triangulates Baileys and Next omissions and exact writable mounts", () => {
    expect(inspectReleaseLayout(completeLayout.filter((path) => path !== "probe/runtime-probe.js")).missing)
      .toEqual(["dynamic-baileys-probe"]);
    expect(inspectReleaseLayout(completeLayout.filter((path) => !path.includes(".next/static"))).missing)
      .toEqual(["next-static"]);
