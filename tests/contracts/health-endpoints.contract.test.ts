...
describe("release health endpoint contracts", () => {
	test("RED: API live is provider-independent and ready returns only coded 200/503 JSON", async () => {
		const pools = {
			api: {
				run: async (
					_context: unknown,
					operation: (repo: {
						releaseReadiness: () => Promise<{ ready: boolean; code: string }>;
					}) => Promise<unknown>,
				) =>
					operation({
		...
		expect(
			(await app.inject({ method: "GET", url: "/internal/live" })).statusCode,
		).toBe(200);
	...
	test("RED: manager and worker wire loopback readiness to durable draining state", async () => {
		const probe = await readFile(
			"packages/runtime-config/src/operations.ts",
			"utf8",
		);
		expect(probe).toContain('host: "127.0.0.1"');
		expect(probe).toContain("port: options.port ?? 9090");
		for (const [file, service] of [
			["apps/message-worker/src/index.ts", "message-worker"],
			["apps/whatsapp-manager/src/index.ts", "whatsapp-manager"],
		] as const) {
