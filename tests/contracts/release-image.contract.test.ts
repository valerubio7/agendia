...
describe("universal release image", () => {
	...
	test("dispatches every supported release command from generated output only", () => {
		...
		expect(dispatcher).toMatch(
			/queue-init\)\n\s*exec bun \/opt\/agendia\/queue-init\/queue-init\.js/,
		);
	});

	test("keeps runtime queue lifecycle separate from the manager publisher credential", () => {
		const worker = read("apps/message-worker/src/index.ts");
		const manager = read("apps/whatsapp-manager/src/index.ts");
		expect(worker).not.toMatch(/createQueue\(/);
		expect(manager).not.toMatch(/createQueue\(/);
		const queueStartup = manager.slice(
			manager.indexOf("export async function startWhatsAppManager"),
			manager.indexOf('if (process.env.AGENDIA_RUN_WHATSAPP_MANAGER === "1")'),
		);
		expect(queueStartup).toContain("QUEUE_PUBLISHER_DATABASE_URL_FILE");
		expect(queueStartup).not.toMatch(
			/env\.(?:WORKER_DATABASE_URL|DATABASE_URL)\b/,
		);
	});
