...
test("admin to tenant to single link to message to deterministic AI to ACK", async ({ page }) => {
  const result = await runDeterministicV1Journey();
  await page.setContent(`<main><h1>Aceptación AgendIA v1</h1><pre>${JSON.stringify(result)}</pre></main>`);
  await expect(page.getByRole("heading", { name: "Aceptación AgendIA v1" })).toBeVisible();
