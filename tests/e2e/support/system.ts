...
export async function startSystem(): Promise<SystemHarness> {
	...
	try {
		...
		const queuePublisherDirectory = await mkdtemp(
			join(tmpdir(), "agendia-system-e2e-queue-publisher-"),
		);
		cleanup.push(() =>
			rm(queuePublisherDirectory, { recursive: true, force: true }),
		);
		await chmod(queuePublisherDirectory, 0o700);
		const queuePublisherFile = join(queuePublisherDirectory, "database-url");
		await writeFile(queuePublisherFile, database.container.getConnectionUri(), {
			mode: 0o600,
		});
		...
		const connectionString = database.container.getConnectionUri();
		...
		await provisionRoles(database.sql, {
			...
			credentials: Object.fromEntries(
				Object.values(roles).map((role) => [role, "system-e2e-role-password"]),
			),
			manageDatabaseOwnership: false,
		});
		await initializeQueues({ sql: database.sql, connectionString });
		...
		const [apiPort, webPort, managerProbePort, workerProbePort] =
			await freePorts(4);
		...
		const managerEnv = {
			...
			LOOPBACK_PROBE_PORT: String(managerProbePort),
			QUEUE_PUBLISHER_DATABASE_URL_FILE: queuePublisherFile,
		};
		const workerEnv = {
			...common,
			DEEPSEEK_API_KEY: "E2E_PROVIDER_SECRET",
			DEEPSEEK_MODEL: "deterministic",
			LOOPBACK_PROBE_PORT: String(workerProbePort),
		};
		...
		const logs: string[] = [];
		const nextEnvDeclarationPath = join(
			process.cwd(),
			"apps/web/next-env.d.ts",
		);
		const nextEnvDeclaration = await readFile(nextEnvDeclarationPath);
		const web = spawn(
		...
		cleanup.push(async () => {
			if (web.exitCode === null) {
				web.kill("SIGTERM");
				await Promise.race([
					new Promise<void>((resolve) => web.once("exit", resolve)),
					delay(5_000),
				]);
				if (web.exitCode === null) {
					web.kill("SIGKILL");
			...
			await writeFile(nextEnvDeclarationPath, nextEnvDeclaration);
		});
