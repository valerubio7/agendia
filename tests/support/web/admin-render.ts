...
export function renderAdminDashboard(
  role: AdminRole,
  businesses: readonly BusinessProjection[],
): string {
  if (role !== "platform_admin")
    return '<main><p role="alert">No tenés acceso al panel administrativo.</p></main>';
  const rows = businesses
    .map(
      (business) =>
        `<tr><td>${escapeHtml(business.name)}</td><td>${label[business.status]}</td><td>${label[business.assistantStatus]}</td><td>${label[business.whatsappStatus]}</td><td>${escapeHtml(business.createdAt)}</td><td>${escapeHtml(business.lastTechnicalActivityAt ?? "Sin actividad")}</td></tr>`,
    )
    .join("");
