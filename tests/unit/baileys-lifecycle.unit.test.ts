...
import { InMemoryWhatsAppConnections, WhatsAppConnectionService } from "../../packages/domain/src/whatsapp/connection.ts";
import { registerWhatsAppRoutes } from "../../apps/api/src/routes/me-whatsapp.ts";
import {
...
function harness(events: Parameters<DeterministicBaileysDouble["script"]>[0]) {
  ...
  return { connections, gateway, commands, links, manager: new WhatsAppLifecycleManager(connections, gateway, commands, links) };
}
...
describe("persistent Baileys linking lifecycle", () => {
  test("processes a durable link command, exposes an expiring QR only to its tenant and opens", async () => {
    const h = harness([{ type: "qr", value: "temporary-qr" }, { type: "open", linkedNumber: "+549111" }]);
    ...
    expect(h.links.consume("tenant-a", "wa-tenant-a", 1_001)).toBe("temporary-qr");
    expect(h.links.consume("tenant-a", "wa-tenant-a", 301_001)).toBeNull();
    expect(h.connections.get("tenant-a")).toMatchObject({ state: "CONNECTED", linkedNumber: "+549111" });
  });

  test("reconnects transient closures with bounded deterministic backoff", async () => {
    const h = harness([{ type: "open", linkedNumber: "+549111" }, { type: "transient-close" }]);
    h.manager.requestLink("tenant-a");
  ...
  test("logout requires a new link while corrupt auth becomes an observable error", async () => {
    const logout = harness([{ type: "open", linkedNumber: "+549111" }, { type: "logout" }]);
    logout.manager.requestLink("tenant-a"); await logout.manager.processNext(3_000);
    expect(logout.connections.get("tenant-a")?.state).toBe("LINK_REQUIRED");
    const corrupt = harness([{ type: "corrupt" }]);
    corrupt.manager.requestLink("tenant-a"); await corrupt.manager.processNext(3_000);
  ...
  test("restores eligible persisted sessions after a manager restart and rejects a second link", async () => {
    ...
    h.manager.requestLink("tenant-a"); await h.manager.processNext(4_000);
    const restarted = new WhatsAppLifecycleManager(h.connections, h.gateway, h.commands, h.links);
    expect(restarted.restorePersistedSessions(["tenant-a", "tenant-missing"])).toEqual(["wa-tenant-a"]);
  ...
  test("serves link commands and ephemeral QR with no-store headers", async () => {
    ...
    registerWhatsAppRoutes(app, h.connections, () => "tenant-a", {
      ...
      consumeLink: (tenant, now) => h.links.consume(tenant, `wa-${tenant}`, now),
      now: () => 5_001,
    });
    expect((await app.inject({ method: "POST", url: "/me/whatsapp/link" })).statusCode).toBe(202);
    await h.manager.processNext(5_000);
    const response = await app.inject({ method: "GET", url: "/me/whatsapp/link" });
