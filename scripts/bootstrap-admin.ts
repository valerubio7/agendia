...
export function releaseAdminInput(
	env: Record<string, string | undefined>,
	readFile: (path: string) => string = (path) => readFileSync(path, "utf8"),
) {
	const release =
		env.AGENDIA_ENVIRONMENT === "staging" ||
		env.AGENDIA_ENVIRONMENT === "production";
	if (!release)
		return validateAdminInput(
			env.AGENDIA_ADMIN_EMAIL,
			env.AGENDIA_ADMIN_PASSWORD,
		);
	if (Object.hasOwn(env, "AGENDIA_ADMIN_PASSWORD"))
		throw new Error("AGENDIA_ADMIN_PASSWORD is forbidden");
	if (!env.AGENDIA_ADMIN_PASSWORD_FILE)
		throw new Error("AGENDIA_ADMIN_PASSWORD_FILE is required");
	let password: string;
	try {
		password = readFile(env.AGENDIA_ADMIN_PASSWORD_FILE).trim();
	} catch {
		throw new Error("AGENDIA_ADMIN_PASSWORD_FILE is unavailable");
	}
	return validateAdminInput(env.AGENDIA_ADMIN_EMAIL, password);
}

export async function bootstrapAdminFromEnvironment(
	env: NodeJS.ProcessEnv = process.env,
) {
	const input = releaseAdminInput(env);
	const release =
		env.AGENDIA_ENVIRONMENT === "staging" ||
		env.AGENDIA_ENVIRONMENT === "production";
	const config = release
		? loadRuntimeConfig("bootstrap-admin", env)
		: undefined;
	if (config) await preflightReleaseEnvironment(config, env);
	const databaseUrl = config?.databaseUrl ?? env.DATABASE_URL;
	if (!databaseUrl) throw new Error("DATABASE_URL is required");
	const store = new PostgresAdminStore(postgres(databaseUrl, { max: 1 }));
	try {
