...
async function dumpData(
	...
	if (
		dump.exitCode !== 0 ||
		!dump.stdout.includes("INSERT INTO public.businesses")
	) {
...
async function restoreData(
	...
	await container.copyContentToContainer([
		{ content: dump, target: "/tmp/agendia-restore.sql" },
	]);
	...
	if (restored.exitCode !== 0)
		throw new Error(`Backup restore failed: ${restored.stderr}`);
}
...
async function encryptedAuthSnapshot(
	sql: Sql,
): Promise<EncryptedAuthSnapshot[]> {
...
export async function runRestoreDrill(): Promise<RestoreDrillReport> {
	...
	try {
		...
		target = await new PostgreSqlContainer(postgresImage).start();
		targetSql = postgres(target.getConnectionUri(), { max: 3 });
		await migrate(targetSql);
		await restoreData(target, dump, targetSql);

		const counts = await targetSql<
			{ business_id: string; count: string }[]
		>`select business_id::text, count(*)::text from messages group by business_id order by business_id`;
		const authForTenantA = await targetSql.begin(async (tx) => {
			...
			return tx<
				{ business_id: string; ciphertext: Uint8Array }[]
			>`select business_id::text, ciphertext from whatsapp_auth_records`;
		});
		const pendingJobs = await targetSql<
			{ count: string }[]
		>`select count(*)::text from outbox_events where topic = 'ai.generate' and published_at is null`;
		const kekVersions = await targetSql<
			{ kek_version: string }[]
		>`select distinct kek_version from whatsapp_connections where kek_version is not null order by kek_version`;
		const targetAuth = await encryptedAuthSnapshot(targetSql);

		return {
			tenantMessageCounts: Object.fromEntries(
				counts.map((row) => [row.business_id, Number(row.count)]),
			),
			tenantAVisibleAuthRecords: authForTenantA.length,
			crossTenantAuthRecords: authForTenantA.filter(
				(row) => row.business_id !== tenantA,
			).length,
			pendingJobs: Number(pendingJobs[0]?.count ?? 0),
			restoredAuthRecords: targetAuth.length,
			authCiphertextsMatchBackup:
				JSON.stringify(targetAuth) === JSON.stringify(sourceAuth),
